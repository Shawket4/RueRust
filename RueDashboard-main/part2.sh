#!/usr/bin/env bash
# =============================================================================
#  Rue POS Dashboard — Frontend Rewrite Part 2: Auth + Dashboard
#  Run from the React project root AFTER part1.sh
#
#  Creates:
#   - src/pages/auth/Login.tsx        (polished login, RTL-aware)
#   - src/pages/dashboard/Dashboard.tsx (role-aware, parallel fetching,
#                                        branch cards, sales summary,
#                                        low stock, recent orders)
# =============================================================================
set -euo pipefail

BOLD='\033[1m'; GREEN='\033[0;32m'; CYAN='\033[0;36m'; RESET='\033[0m'
log() { echo -e "${CYAN}[part2]${RESET} $*"; }
...
mkdir -p src/pages/auth src/pages/dashboard

# ===========================================================================
#  src/pages/auth/Login.tsx
# ===========================================================================
log "Writing Login page ..."
cat > src/pages/auth/Login.tsx << 'TSX'
...
# ===========================================================================
log "Writing Dashboard page ..."
cat > src/pages/dashboard/Dashboard.tsx << 'TSX'
...
import {
  ...
  ArrowRight, Package, RefreshCw, Wifi, WifiOff,
} from "lucide-react";
import { cn } from "@/lib/utils";
...
import {
  egp, fmtTime, fmtDuration, fmtPayment,
  PAYMENT_BG, initials,
} from "@/utils/format";
...
function StatCard({ icon: Icon, label, value, sub, accent = "text-primary", bg = "bg-primary/10", loading, to }: StatCardProps) {
  const inner = (
    <Card className={cn("p-5 h-full flex flex-col justify-between transition-all hover:shadow-md group", to && "cursor-pointer")}>
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          <span className={cn(
            ...
            shiftLoading ? "bg-muted-foreground animate-pulse" :
            hasOpen      ? "bg-green-500 shadow-[0_0_0_3px_rgba(34,197,94,0.25)]" :
                           "bg-muted-foreground",
          )} />
          <p className="font-semibold text-sm truncate">{branch.name}</p>
        </div>
        {!shiftLoading && (
          <Badge variant={hasOpen ? "success" : "secondary"} className="flex-shrink-0 ml-2">
            {hasOpen ? "Open" : "Closed"}
          </Badge>
        )}
      </div>
        ...
        {shiftLoading ? (
          ...
          <>
            <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
              ...
              <span className="font-semibold text-right truncate">{openShift.teller_name}</span>
              <span className="text-muted-foreground">Running</span>
              <span className="font-mono text-right">{fmtDuration(openShift.opened_at)}</span>
              <span className="text-muted-foreground">Opening</span>
              <span className="font-mono text-right">{egp(openShift.opening_cash)}</span>
            </div>
            <div className="pt-2 border-t border-border">
              <div className="flex items-center justify-between">
                <span className="text-xs text-muted-foreground">Today's Sales</span>
                <span className="font-bold text-primary text-sm font-mono">{egp(todaySales)}</span>
              </div>
              <div className="flex items-center justify-between mt-0.5">
                <span className="text-xs text-muted-foreground">{validOrds.length} orders</span>
          ...
          <p className="text-sm text-muted-foreground text-center py-1">No active shift</p>
        )}
  ...
  return (
    <Card className="overflow-hidden">
      <div className="px-5 py-3.5 border-b border-border flex items-center justify-between">
        ...
        <Link to="/shifts" className="text-xs font-semibold text-primary hover:text-primary/80 flex items-center gap-1 flex-shrink-0 ml-2">
      ...
      {isLoading ? (
        ...
        <div className="divide-y divide-border overflow-y-auto" style={{ maxHeight: "min(360px,50dvh)" }}>
          {recent.map((order) => {
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className={cn(
                      ...
                      PAYMENT_BG[order.payment_method] ?? "bg-muted text-muted-foreground",
                    )}>
...
function LowStockPanel({ branches }: { branches: Branch[] }) {
    ...
    queryFn:   async () => {
      ...
      results
        ...
        .forEach((item) => {
          ...
          if (
            !existing ||
            parseFloat(String(item.current_stock)) < parseFloat(String(existing.current_stock))
          ) {
  ...
  const low = items
    .filter((i) => parseFloat(String(i.current_stock)) <= parseFloat(String(i.reorder_threshold)))
    .sort(
      (a, b) =>
        parseFloat(String(a.current_stock)) / parseFloat(String(a.reorder_threshold)) -
        parseFloat(String(b.current_stock)) / parseFloat(String(b.reorder_threshold)),
    );
  ...
  return (
    <Card className="overflow-hidden">
      <div className="px-5 py-3.5 border-b border-border flex items-center justify-between">
        ...
        <Link to="/inventory" className="text-xs font-semibold text-primary hover:text-primary/80 flex items-center gap-1 flex-shrink-0 ml-2">
...
function SalesSummary({ branches }: { branches: Branch[] }) {
  const { data, isLoading } = useQuery({
    ...
    staleTime: 60_000,
    queryFn:   async () => {
      let totalSales = 0, totalOrders = 0, cash = 0, card = 0, talabat = 0, openCount = 0;
      await Promise.allSettled(
        branches.map(async (branch) => {
          try {
            ...
            openCount++;
            const orders = await getOrders({ shift_id: s.open_shift.id }).then((r) => r.data);
            const valid  = (orders as Order[]).filter((o) => o.status !== "voided");
            ...
            valid.forEach((o) => {
              ...
              if (o.payment_method === "talabat_online" ||
                  o.payment_method === "talabat_cash")   talabat  += o.total_amount;
            });
          } catch { /* skip */ }
        }),
      );
  ...
  const items = [
    { label: "Total Sales",   value: egp(data?.totalSales),  className: "text-primary"   },
    { label: "Orders",        value: data?.totalOrders ?? 0, className: "text-green-600" },
    { label: "Cash",          value: egp(data?.cash),        className: "text-foreground" },
    { label: "Card",          value: egp(data?.card),        className: "text-violet-600" },
    { label: "Talabat",       value: egp(data?.talabat),     className: "text-orange-600" },
  ];
    ...
    <Card className="overflow-hidden">
      <div className="px-5 py-3.5 border-b border-border flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {!isLoading && data && (
            <span className="text-xs text-muted-foreground">{data.openCount} active</span>
          )}
        </div>
        <Link to="/shifts" className="text-xs font-semibold text-primary hover:text-primary/80 flex items-center gap-1">
        ...
        {items.map(({ label, value, className }) => (
          <div key={label} className="px-4 py-4">
            {isLoading ? (
              ...
              <>
                <p className={cn("text-base font-extrabold font-mono", className)}>{value}</p>
  ...
  return (
    <div className="flex items-center gap-3 px-5 py-3">
      <span className={cn(
        ...
        hasOpen   ? "bg-green-500" : "bg-muted-foreground",
      )} />
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium truncate">{branch.name}</p>
        {!isLoading && hasOpen && shift && (
          <p className="text-xs text-muted-foreground truncate">{shift.teller_name} · {fmtDuration(shift.opened_at)}</p>
        )}
        {!isLoading && !hasOpen && <p className="text-xs text-muted-foreground">No active shift</p>}
      </div>
      {isLoading ? (
...
// ── Active shifts count ───────────────────────────────────────────────────────
function ActiveShiftsCard({ branches, loading }: { branches: Branch[]; loading: boolean }) {
...
function SuperAdminStats() {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      <StatCard icon={Building2} label="Organizations" value={orgs?.length} sub="Active brands"   accent="text-primary"   bg="bg-primary/10"      loading={orgsLoading}  to="/orgs"  />
      <StatCard icon={Users}     label="Total Users"   value={users?.length} sub="Staff accounts" accent="text-violet-600" bg="bg-violet-100 dark:bg-violet-950/50" loading={usersLoading} to="/users" />
      <StatCard icon={GitBranch} label="Active Orgs"   value={orgs?.filter((o) => o.is_active).length}  sub="Live now"    accent="text-amber-600"  bg="bg-amber-100 dark:bg-amber-950/50"  loading={orgsLoading}  />
      <StatCard icon={Coffee}    label="Active Users"  value={users?.filter((u) => u.is_active).length} sub="Accounts"    accent="text-green-600"  bg="bg-green-100 dark:bg-green-950/50"  loading={usersLoading} />
    </div>
  );
}
...
function OrgAdminStats({ orgId }: { orgId: string }) {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      <StatCard icon={Users}     label="Staff"    value={users?.length}    sub={`${users?.filter((u) => u.is_active).length ?? 0} active`}   accent="text-violet-600" bg="bg-violet-100 dark:bg-violet-950/50" loading={usersLoading}    to="/users"    />
      <StatCard icon={GitBranch} label="Branches" value={branches?.length} sub={`${branches?.filter((b) => b.is_active).length ?? 0} active`} accent="text-primary"   bg="bg-primary/10"      loading={branchesLoading} to="/branches" />
      <StatCard icon={Coffee}    label="Operating" value={branches?.filter((b) => b.is_active).length} sub="Today" accent="text-amber-600" bg="bg-amber-100 dark:bg-amber-950/50" loading={branchesLoading} />
      ...
      {isSuperAdmin && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <Card className="p-5 sm:p-6">
            ...
            <div className="space-y-2">
              {[
                { label: "Manage Organizations", to: "/orgs",      color: "bg-primary/10 text-primary hover:bg-primary/20 dark:bg-primary/20"           },
                { label: "Manage Users",          to: "/users",     color: "bg-violet-100 text-violet-700 hover:bg-violet-200 dark:bg-violet-950/50"     },
                { label: "View All Branches",     to: "/branches",  color: "bg-amber-100 text-amber-700 hover:bg-amber-200 dark:bg-amber-950/50"         },
                { label: "Shifts & Reports",      to: "/shifts",    color: "bg-green-100 text-green-700 hover:bg-green-200 dark:bg-green-950/50"         },
                { label: "Analytics",             to: "/analytics", color: "bg-blue-100 text-blue-700 hover:bg-blue-200 dark:bg-blue-950/50"             },
              ].map((a) => (
                <Link
                  key={a.label}
                  to={a.to}
...
echo ""
echo -e "${BOLD}═══════════════════════════════════════════════════════════════${RESET}"
echo -e "${GREEN}  Part 2 complete! Auth + Dashboard ready.${RESET}"
echo -e "${BOLD}═══════════════════════════════════════════════════════════════${RESET}"
echo ""
echo "  Created:"
echo "    ✓ src/pages/auth/Login.tsx"
echo "      - Split panel (hero left, form right)"
echo "      - Feature cards with icons"
echo "      - RTL-aware layout"
echo "      - Sonner toast on error"
echo "      - Zustand signIn on success"
echo ""
echo "    ✓ src/pages/dashboard/Dashboard.tsx"
echo "      - Role-aware: super_admin / org_admin / branch_manager"
echo "      - Welcome hero with brand gradient"
echo "      - Stat cards (orgs, users, branches, active shifts)"
echo "      - Branch grid with live shift status"
echo "      - Today's sales breakdown (cash/card/talabat)"
echo "      - Recent orders panel (auto-scans for open shift)"
echo "      - Low stock panel with progress bars"
echo "      - Branch status row list"
echo "      - All parallel fetching via Promise.allSettled"
echo "      - Dark mode, responsive mobile/tablet/desktop"
echo ""
echo "  Run: npm run dev  to see it"
echo "  Then: bash frontend_part3.sh  (Users + Orgs + Branches + Permissions)"
echo ""